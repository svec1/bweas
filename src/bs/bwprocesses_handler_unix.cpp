...
void processes_handler::start(std::function<void(string_v, bool)> do_more_func) {
    for (size_t i = 0; i < cmd_s.size(); ++i, ++count_runable_processes) {
        if (count_runable_processes >= max_count_processes) {
            ...
            const auto &completed_cmd = std::find_if(cmd_s.begin(), cmd_s.end(), [tmp_pid](const command &cmd) {
        ...
        for (const auto &name_dependence : cmd_s[i].depends_command) {
            const auto &dependence_cmd =
                std::find_if(cmd_s.begin(), cmd_s.end(),
                             [name_dependence](const command &cmd) { return cmd.name == name_dependence; });
...
wait_all_process:
    while (count_runable_processes) {
        const auto &it = *std::find_if(cmd_s.begin(), cmd_s.end(), [tmp_pid = wait_process()](const command &cmd) {
            return cmd.pid_execute_process == tmp_pid;
        });
