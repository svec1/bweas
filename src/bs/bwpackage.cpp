...
string package::create_data_package(data_bw_package _data) {
    ...
    for (const auto &src_lua_generator : _data.src_lua_generators)
        data_str += src_lua_generator + PACKAGE_SEPARATE_LUA_DEPENDENCY_FINDERS;
...
string package::init(data_bw_package _data, bool is_create_pckg) {
    ...
    if (config_json.contains("dependency-finder")) {
        ...
        for (const auto &finder : config_json["dependency-finder"].items()) {
            nlohmann::json metainf_gn = finder.value();

            if (!metainf_gn.is_object())
                _log << (log_message(log_type::fatal)
                         << "The dependency-finder meta information unit must be a json object");
            else if (!metainf_gn.contains("lua-file") || !metainf_gn["lua-file"].is_string())
                _log << (log_message(log_type::fatal) << "dependency-finder metadata must include the path to the lua "
                                                         "(finder) source code file");

            auto lua_file_finder_source = bwtools::open_file((string)metainf_gn["lua-file"]);
            if (is_create_pckg) {
                cfg_package.finders.emplace_back(finder.key(),
                                                 bwtools::read_file(bwtools::get_ref_file(lua_file_finder_source)));
                _data.src_lua_finders.push_back(cfg_package.finders[cfg_package.finders.size() - 1].src_lua);
            }
            else {
                cfg_package.finders.emplace_back(finder.key(), _data.src_lua_finders[i]);
                ++i;
            }
        }
    }
    if (config_json.contains("modules")) {
        ...
        for (const auto &module : config_json["modules"].items()) {
            ...
            cfg_package.modules.emplace_back(module.key(), metainf_md["lua-file"], funcs);
        }
    }
...
void package::load(string_v raw_data_package) {
    ...
    while (src_finders.size()) {
        ...
        src_finders.erase(0, current_src_finder.size() + PACKAGE_SEPARATE_LUA_GENERATE_LENGHT);
    }
