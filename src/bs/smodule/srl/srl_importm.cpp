...
#define call_err_tk_inf_lex(count_line, count_sym, lexem, expected, err)                                               \
    assist.call_err(err, "\n(Line: " + std::to_string(count_line) +                                                    \
                             "; Symbols start pos: " + std::to_string(count_sym - lexem.size()) + "): Lexem - \"" +    \
                             lexem + "\"" + "\n Expected: " + expected + "\n");

#define call_err_identify_expected()                                                                                   \
    if (expected_close_ang_br && expected_start_module)                                                                \
    call_err_tk_inf_lex(count_line, count_sym, lexem, "<start_module>",                                                \
                        "SRL-IMP000") else if (expected_close_ang_br && expected_end_module)                           \
        call_err_tk_inf_lex(count_line, count_sym, lexem, "<end_module>",                                              \
                            "SRL-IMP000") else if (expected_qouts && expected_enum_params)                             \
            call_err_tk_inf_lex(count_line, count_sym, lexem, "SYMBOL \'\"\'",                                         \
                                "SRL-IMP000") else if (expected_separator && expected_filename_dll)                    \
                call_err_tk_inf_lex(count_line, count_sym, lexem, "SYMBOL \':\'",                                      \
                                    "SRL-IMP000") else if (expected_separator && expected_func_name)                   \
                    call_err_tk_inf_lex(count_line, count_sym, lexem, "OPERATOR \'->\'",                               \
                                        "SRL-IMP000") else if (expected_separator && expected_enum_params)             \
                        call_err_tk_inf_lex(count_line, count_sym, lexem, "OPERATOR \',\'",                            \
                                            "SRL-IMP000") else if (expected_filename_dll)                              \
                            call_err_tk_inf_lex(count_line, count_sym, lexem, "filename.dll",                          \
                                                "SRL-IMP000") else if (expected_func_name)                             \
                                call_err_tk_inf_lex(count_line, count_sym, lexem,                                      \
                                                    "func_name: ", "SRL-IMP000") else if (expected_enum_params)        \
                                    call_err_tk_inf_lex(count_line, count_sym, lexem, "PARAM[INT,STRING...]",          \
                                                        "SRL-IMP000") else if (expected_open_br)                       \
                                        call_err_tk_inf_lex(count_line, count_sym, lexem, "SYMBOL \'(\'",              \
                                                            "SRL-IMP000") else if (expected_close_br)                  \
                                            call_err_tk_inf_lex(count_line, count_sym, lexem, "SYMBOL \')\'",          \
                                                                "SRL-IMP000")
...
srl::parser(std::string symbols) {
    ...
    while ((ch = get(symbols, pos))) {
        ...
        if (ch == ' ' || ch == '\n' || ch == '\r') {
            ...
            if ((expected_close_ang_br && !lexem.empty() && symbols[pos - 1] != '>') ||
                (!expected_close_ang_br && !lexem.empty() && !expected_qouts))
                expected_separator = 1;
        }
        if (ch == '>') {
            if (!expected_close_ang_br && !expected_func_name)
                call_err_identify_expected() expected_close_ang_br = 0;
            if (lexem == "start_module") {
                if (!expected_start_module)
                    call_err_identify_expected() expected_start_module = 0;
                ...
                if (!expected_end_module || expected_start_module)
                    call_err_identify_expected() expected_start_module = 1;
            ...
            if (!expected_filename_dll)
                call_err_identify_expected()
                    ...
                    if (!expected_qouts) expected_qouts = 1;
            else
                expected_qouts = 0;
            continue;
        }
            ...
            if (ch == ':') {
                if (!expected_filename_dll || lexem.empty())
                    call_err_identify_expected() expected_filename_dll = 0;
                ...
                if (!expected_func_name || lexem.empty())
                    call_err_identify_expected() expected_func_name = 0;
                expected_separator = 0;
                ...
                if (!expected_open_br || !lexem.empty())
                    call_err_identify_expected() expected_open_br = 0;
                expected_close_br = 1;
                ...
                if (!expected_close_br || lexem.empty())
                    call_err_identify_expected() expected_close_br = 0;
                if (lexem == "SMT")
        ...
        if (expected_separator)
            call_err_identify_expected() lexem += ch;
    }
    ...
    for (u32t i = 1; i < tokens.size(); ++i) {
        if (tokens[i].token_t == token_type::NAME_MODULE) {
            ...
            if (tokens[i].token_p == aef_expr::params::SIZE_ENUM_PARAMS)
                call_err_tk_inf_lex(tokens[i].lines, tokens[i].symbols, tokens[i].val,
                                    "params -> [LIT_NUM, LIT_STR, VAR_ID, "
                                    "FUTURE_VAR_ID, VAR_STRUCT_ID, "
                                    "ANY_VALUE_WITHOUT_FUTUREID_NEXT, "
                                    "LNUM_OR_ID_VAR, LSTR_OR_ID_VAR, "
                                    "NEXT_TOO]",
                                    "SRL-IMP001") tmp_func[tmp_name_func]
                    .push_back(tokens[i].token_p);
        }
