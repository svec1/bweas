...
namespace var{
    ...
    class scope{
        ...
        public:
            template<typename T>
            inline T& create_var(std::string name_var, T val = T{});
            template<typename T>
            inline void delete_var(std::string name_var);
            template<typename T>
    ...
    template<>
    inline std::string& scope::create_var<std::string>(std::string name_var, std::string val){
    ...
    template<>
    inline std::vector<int>& scope::create_var<std::vector<int>>(std::string name_var, std::vector<int> val){
    ...
    template<>
    inline std::vector<std::string>& scope::create_var<std::vector<std::string>>(std::string name_var, std::vector<std::string> val){
    ...
    template<>
    inline void scope::delete_var<int>(std::string name_var){
        if(int_v.delete_var(name_var))
            assist.call_err("SCOP000", "int() <- " + name_var);
    }
    template<>
    inline void scope::delete_var<std::string>(std::string name_var){
        if(str_v.delete_var(name_var))
            assist.call_err("SCOP000", "string() <- " + name_var);
    }
    template<>
    inline void scope::delete_var<struct_sb::project>(std::string name_var){
        if(prj_v.delete_var(name_var))
            assist.call_err("SCOP000", "project() <- " + name_var);
    }
    template<>
    inline void scope::delete_var<struct_sb::target>(std::string name_var){
        if(trg_v.delete_var(name_var))
            assist.call_err("SCOP000", "target() <- " + name_var);
    }
    template<>
    inline void scope::delete_var<std::vector<int>>(std::string name_var){
        if(vec_int_v.delete_var(name_var))
            assist.call_err("SCOP000", "vector<int>() <- " + name_var);
    }
    template<>
    inline void scope::delete_var<std::vector<std::string>>(std::string name_var){
        if(vec_str_v.delete_var(name_var))
            assist.call_err("SCOP000", "vector<std::string>() <- " + name_var);
    }
    template<>
    inline int& scope::get_var_value<int>(std::string name_var){
        if(!int_v.is_exist_var(name_var)) assist.call_err("SCOP000", "int() <- " + name_var);
        return int_v.get_val_ref(name_var);
    }
    template<>
    inline std::string& scope::get_var_value<std::string>(std::string name_var){
        if(!str_v.is_exist_var(name_var)) assist.call_err("SCOP000", "string() <- " + name_var);
        return str_v.get_val_ref(name_var);
    }
    template<>
    inline struct_sb::project& scope::get_var_value<struct_sb::project>(std::string name_var){
        if(!prj_v.is_exist_var(name_var)) assist.call_err("SCOP000", "project() <- " + name_var);
        return prj_v.get_val_ref(name_var);
    }
    template<>
    inline struct_sb::target& scope::get_var_value<struct_sb::target>(std::string name_var){
        if(!trg_v.is_exist_var(name_var)) assist.call_err("SCOP000", "target() <- " + name_var);
        return trg_v.get_val_ref(name_var);
    }
    template<>
    inline std::vector<int>& scope::get_var_value<std::vector<int>>(std::string name_var){
        if(!vec_int_v.is_exist_var(name_var)) assist.call_err("SCOP000", "vector<int>() <- " + name_var);
        return vec_int_v.get_val_ref(name_var);
    }
    template<>
    inline std::vector<std::string>& scope::get_var_value<std::vector<std::string>>(std::string name_var){
        if(!vec_str_v.is_exist_var(name_var)) assist.call_err("SCOP000", "vector<string>() <- " + name_var);
        return vec_str_v.get_val_ref(name_var);
    }

    inline bool scope::is_exist(std::string name_var){
        if(int_v.is_exist_var(name_var) || str_v.is_exist_var(name_var) ||
           vec_int_v.is_exist_var(name_var) || vec_str_v.is_exist_var(name_var) ||
           prj_v.is_exist_var(name_var) || trg_v.is_exist_var(name_var)) return 1;
        return 0;
    }

    inline u32t scope::what_type(std::string name_var){
        if(int_v.is_exist_var(name_var)) return 1;
        else if(str_v.is_exist_var(name_var)) return 2;
        else if(vec_int_v.is_exist_var(name_var)) return 3;
        else if(vec_str_v.is_exist_var(name_var)) return 4;
        else if(prj_v.is_exist_var(name_var)) return 5;
        else if(trg_v.is_exist_var(name_var)) return 6;
        else return 0;
    }
