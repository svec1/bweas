...
template <typename T> class datatype_var {
  public:
    ...
    datatype_var<T> &
    operator=(const datatype_var<T> &dt_var);

    ~datatype_var() = default;

  public:
    const std::pair<std::string, T> &
    create_var_r(std::string name_var, T value = {});
    u32t
    create_var(std::string name_var, T value = NULL);
    ...
    u32t
    copy(std::string name_var, std::string name_var_src);

    T
    get_value(std::string name_var);

    const std::pair<std::string, T> &
    get_ref(std::string name_var);
    T &
  ...
  private:
    u32t
    find_var_ind(std::string name_var);
...
template <typename T>
u32t
datatype_var<T>::find_var_ind(std::string name_var) {
...
template <typename T>
bool
datatype_var<T>::is_exist_var(std::string name_var) {
...
template <typename T>
const std::pair<std::string, T> &
datatype_var<T>::create_var_r(std::string name_var, T value) {
...
template <typename T>
u32t
datatype_var<T>::create_var(std::string name_var, T value) {
    if (is_exist_var(name_var))
        return 1;
...
template <typename T>
u32t
datatype_var<T>::delete_var(std::string name_var) {
    if (!is_exist_var(name_var))
        return 1;
...
template <typename T>
u32t
datatype_var<T>::copy(std::string name_var, std::string name_var_src) {
    if (!is_exist_var(name_var))
        return 1;
...
template <typename T>
T
datatype_var<T>::get_value(std::string name_var) {
    if (!is_exist_var(name_var))
        return T();
...
template <typename T>
const std::pair<std::string, T> &
datatype_var<T>::get_ref(std::string name_var) {
