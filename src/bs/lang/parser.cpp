...
std::string parser::build_pos_tokenb_str(const lexer::token& tk){
    if(!tk.token_val.empty())
        return "\n(Line: " + std::to_string(tk.pos_defined_line) +
               ...
               "): Type token - " + type_token_str(tk.token_t) +
               "; Token value - \"" + tk.token_val + "\"\n";
    return "\n(Line: " + std::to_string(tk.pos_defined_line) +
           "; Symbols start pos: " + std::to_string(tk.pos_beg_defined_sym) +
           "): Type token - " + type_token_str(tk.token_t) + "\n";
}
...
std::string parser::tree_build_visually_str(const abstract_expr_func& aef){
    ...
    for(u32t i = 0; i < aef.size(); ++i){
        ...
        tree_visible << " |TOKEN FUNC: " << "TYPE TOKEN: " << parser::type_token_str(aef[i].expr_func.func_t.token_t) << "; LEXEM: " << aef[i].expr_func.func_t.token_val << '\n';
        for(u32t j = 0; j < aef[i].sub_expr_s.size(); ++j){
            ...
            for(u32t k = 0; k < aef[i].sub_expr_s[j].token_of_subexpr.size(); ++k){
                tree_visible << " |  |  | -> " << parser::type_token_str(aef[i].sub_expr_s[j].token_of_subexpr[k].token_t) << "(" << aef[i].sub_expr_s[j].token_of_subexpr[k].token_val << ")\n";
...
pars_an::pars_an(){
    if(!init_glob){
        ...
        assist.add_err("PARS001", "After the start of the expression, a token is expected to begin enumerating arguments with type \"SYMBOL\" and value \"(\"");
        assist.add_err("PARS002", "Function arguments can only be tokens of type (\"ID\", \"LITERAL\" or \"LITERALS\" with the possibility of switching on \"OPERATOR\") <- subexpression");
        ...
        assist.add_err("PARS005", "A token with the type \"ID\", \"LITERAL\" or \"LITERALS\" is expected after a token with the type \"OPERATOR\"");
...
void pars_an::check_valid_subexpr_first_pass(const subexpressions& sub_expr){
    if(sub_expr.token_of_subexpr.empty()) return;
    ...
    for(u32t i = 0; i < sub_expr.token_of_subexpr.size(); ++i){
        if(sub_expr.token_of_subexpr[i].token_t == lexer::token_type::ID ||
           ...
           expected_op = 1;
            ...
            assist.call_err("PARS004", build_pos_tokenb_str(sub_expr.token_of_subexpr[i]));
        }
    }
    if(!expected_op)
        assist.call_err("PARS005", build_pos_tokenb_str(sub_expr.token_of_subexpr[sub_expr.token_of_subexpr.size()-1]));
}
        ...
        if(sub_expr.token_of_subexpr[i].token_t == lexer::token_type::LITERALS){
            if(num_type_expr) {
                if(operator_plus)
                    ...
                    assist.call_err("PARS007", build_pos_tokenb_str(sub_expr.token_of_subexpr[i]));
            }
            str_type_expr = 1;
        }
            ...
            id_type_expr = 1;
                ...
                if(operator_compare)
                    if(str_type_expr)
                        assist.call_err("PARS008", build_pos_tokenb_str(sub_expr.token_of_subexpr[i]));
