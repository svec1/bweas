...
void sl_func::executable(const std::vector<subexpressions>& sub_expr, var::scope& curr_scope){
    if(curr_scope.what_type(sub_expr[2].token_of_subexpr[0].token_val) != 5)
        throw semantic_an::semantic_excp(parser::build_pos_tokenb_str(sub_expr[2].token_of_subexpr[0]) +
    ...
    trg_ref.prj = std::make_shared<var::struct_sb::project>(curr_scope.get_var_value<var::struct_sb::project>(sub_expr[2].token_of_subexpr[0].token_val));
