...
void err_fatal_obj(_err obj_err);
void err_fatal_ref(const _err *obj_err);
...
void kl_reset_track_file(){
    while(krnlFiles.count)
        kl_strack_file(krnlFiles.count-1);
}

const pfile kl_get_track_file(u32t ind){
    if(ind >= krnlFiles.count)
        err_fatal_ref(get_kernel_err(1));
    return &krnlFiles.files[ind];
}
const str* kl_get_desc_track_file(u32t ind){
    if(ind >= krnlFiles.count)
        err_fatal_ref(get_kernel_err(1));
    return &krnlFiles.desc[ind];
}

size32t kl_get_size_list_tf(){
    return krnlFiles.count;
}

str kl_get_track_list(){
...
i32t kl_find_file(str name){
    for(u32t i = 0; i < krnlFiles.count; ++i){
        if(!strcmp(krnlFiles.desc[i].name, name)) return i;
    }
    return -1;
}
i32t kl_find_file_p(const pfile file){
    for(u32t i = 0; i < krnlFiles.count; ++i){
        if(kl_get_track_file(i) == file) return i;
    }
    return -1;
}
...
str kl_build_inf_file(const inf_file* file){
