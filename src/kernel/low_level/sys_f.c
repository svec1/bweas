...
void add_el(void** arr, size32t bit_size, size32t *size){
...
void add_el_c(void** arr, size32t bit_size, size32t size){
    void* arr_new = malloc(bit_size*(size+1));
    for(u32t i = 0; i < bit_size*size; ++i){
        ((u8t*)arr_new)[i] = ((u8t*)(*arr))[i];
    }
...
void del_el(void** arr, size32t bit_size, size32t *size, u32t ind){
    if(ind >= *size)
    ...
    for(u32t i = 0, b = 0; i < bit_size*(*size); ++i){
        if (i >= beg_mempos && i < end_mempos) continue;
...
void del_el_c(void** arr, size32t bit_size, size32t size, u32t ind){
    if(ind >= size)
        err_fatal_ref(get_kernel_err(1));
...
void del_el_last(void** arr, size32t bit_size, size32t *size){
    void* arr_new = malloc(bit_size*(*size-1));
...
void del_el_last_c(void** arr, size32t bit_size, size32t size){
    void* arr_new = malloc(bit_size*(size-1));
    for(u32t i = 0; i < bit_size*(size-1); ++i){
        ((u8t*)arr_new)[i] = ((u8t*)(*arr))[i];
    }
...
str get_time_stm(struct tm *_time){
    char buf_tm[256];
